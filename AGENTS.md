...
5. **學生名單你不看、不念、不轉貼。** 學生真名與家長信箱放在 `data/roster.csv`、`data/contacts.csv`
   ...
   **這些檔你用任何方式都不打開**：讀檔工具、`cat`、`type`、`Get-Content`、`head`、Python 印出來、複製進對話，全部不行。
   `.claude/settings.json`、`.geminiignore`、`.aiexclude` 三處列了同一份禁讀清單（另含 `data/records/**`、
   `data/ledgers/alias-history.jsonl`），但那只是**盡力而為的防呆，不是安全邊界**：有些代理只擋得住讀檔工具，
   `.claude/settings.json` 另外擋了幾個常見的終端機讀檔指令，換個指令或寫段程式照樣讀得到。
...
5. 先問要不要開信箱連結登入（班上家長幾乎都有 Gmail 或 Google 帳號的話，建議不開）：

   >「接下來這個是給**沒有 Google 帳號**的家長用的『寄登入連結到信箱』。先跟你說兩個代價：
   > 第一，打開它之後，任何知道某位家長信箱的人，都能趁那位家長還沒登入過班網時，先用他的信箱註冊一個帳號；
   > 家長要是點了系統寄去的『驗證信箱』信，那個帳號就能用家長的身分登入。所以這個網站讓『信箱連結』登入的人**只能看**
   > 公開的紀事和相簿，不能留言、不能簽已讀、看不到私密紀事和『我的孩子』。
   > 第二，免費方案每天只有 5 封登入信，而且任何知道網址的人都能在登入畫面把這 5 封用光，當天真正的家長就收不到。
   > 班上幾乎都有 Gmail 的話，這一步可以不開。你要打開嗎？」

   他說要開：
   >「同一頁再選『電子郵件/密碼』：打開第一個『啟用』，**再打開下面的『電子郵件連結（無密碼登入）』**，按『儲存』。」

   他說不開：這一題跳過，`progress.json` 的 note 寫「沒開信箱連結」。之後 `doctor.py --cloud` 的「登入方式」那一行
   照樣是 ✓，只在詳情註明「email 連結沒開（選用）」。班上之後出現沒有 Google 帳號的家長，
   先照「沒有 Google 帳號的家長」那一節請他用原本的信箱申請 Google 帳號；真的需要再回來開。
   非 Google 家長很多、又要讓他們留言的班：唯一的正解是升 Blaze、自己加一支 Firebase 的 blocking function
   （`beforeUserCreated` 擋掉 `password` 註冊）——**這個專案沒有內建**，照鐵則 12 如實告訴他。
...
4. 這裡點的設定要到步驟 6 登入後才能從雲端逐項驗（`doctor.py --cloud`）。在那之前靠他念出畫面上的狀態：
   資料庫位置、正式版模式、Google 已啟用、電子郵件連結已啟用（或他決定不開）、方案是 Spark。
...
7. （選用：班上有沒有 Google 帳號的家長時才做）信箱連結登入：先把一個非 Gmail 信箱加進 `contacts.csv` 並同步，
   ...
   驗證：看得到紀事與相簿；留言框停用，寫著「此登入方式只能閱讀，留言與私密內容請用 Google 登入。」（這是對的，不是壞掉）。
...
- [ ] 這台電腦的健檢：必要工具全部 ✓
- [ ] 雲端健檢全部 ✓（決定不開信箱連結的，「登入方式」那一行的詳情會註明 email 連結沒開，這是選用、不影響 ✓），方案那一行顯示 Spark（或 Blaze＋已設預算警示）
...
- [ ] 他知道：免費方案每天只能寄 5 封登入信；留言、私密紀事、「我的孩子」都要 Google 帳號；名單改了要跟你說「同步家長名單」
- [ ] 他知道：導師帳號登入只保留到關掉分頁（別人用同一台電腦就不會直接變成導師）；在學校、圖書館的電腦登入前勾「這是共用電腦」
...
- 「我的孩子」（孩子的部落格與對話）、留言、已讀回條、私密紀事**只接受 Google 登入**；「寄登入連結到信箱」登入的人只能看公開的紀事與相簿。
  原因：信箱連結登入要打開的「電子郵件/密碼」同時開放了密碼註冊，任何人都能拿一位還沒登入過的家長信箱先註冊，
  家長點了驗證信就被冒用；規則分不出這兩種登入（DATA-MODEL §1.1）。
- 免費方案整個網站**每天只能寄 5 封登入信**。一封信登入一台裝置，登入後會一直保持登入；換手機或換電腦要再寄一次。
- **這 5 封任何知道網址的人都能用光**（登入畫面填 5 個隨便的信箱就好），當天沒有 Google 帳號的家長就收不到信。
  防法：開 App Check（reCAPTCHA）擋掉不是從網站送出的請求——但要網頁程式配合，**v0.1 還沒內建**，不要叫老師在主控台直接打開強制
  （一開全站都登不進去）；現在能做的是**不開信箱連結**，只用 Google 登入（DATA-MODEL §5.1）。
...
- 這種家長很多的班：建議升 Blaze（上一節）；還要讓他們留言，得另外加 blocking function 擋密碼註冊（步驟 2 第 5 題，本專案沒有內建）。
